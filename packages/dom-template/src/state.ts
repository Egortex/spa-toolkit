...
export interface State<T> {
	 ...
	 *
	 * ⚠️ `listener` НЕ вызывается немедленно текущим значением при подписке (в отличие,
	 * например, от RxJS `BehaviorSubject`) — только на будущие `set()`. Если DOM должен
	 * отражать значение сразу (а не только его последующие изменения), вызовите
	 * `listener(get())` самостоятельно один раз рядом с `subscribe()`.
	 */
